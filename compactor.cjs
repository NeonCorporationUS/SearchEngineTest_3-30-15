...
// This currently lives in /htdocs/includes/head_js/links_google.js until we can come up with a clean solution for a second compactor in the head.
...
$(function() {
	...
	var display = function($module) {
		$module.closest('.links_google_csa').css('visibility', 'visible');
	};

	var hide = function() {
		_.each(moduleSelectors, function(selector) {
			$(selector).closest('.links_google_csa').hide();
		});
	};

	var removeInterval = function(interval) {
		window.clearInterval(interval);
	};

	var afterInitialize = function() {
		var interval = window.setInterval(function() {
			if (_.isEmpty(moduleSelectors)) { removeInterval(interval); }

			_.each(moduleSelectors, function(selector, index) {
				if ($(selector).find('iframe').height() > 0) {
					display($(selector));
					moduleSelectors.splice(index, 1);
				}
			});
		}, 500);
		...
		window.setTimeout(function() {
			...
			hide(); // collapse any remaining unfilled modules
		}, 10000);
	};

	var initialize = function() {
		var i = 0;
		while (pageData['links_google_csa-' + i]) {
			selector = '.' + 'csa_block_' + (i + 1);
			if (pageData['links_google_csa-' + i]['collapse_empty_divs'] === 'yes') {
